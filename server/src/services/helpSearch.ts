...
Use Smart Search at the top of the homepage. It supports plain keywords and titles (prompts and context also search body text; skills also match install URL), natural-language queries with facet hints (e.g. "cursor prompts for code review" — tools, prompt/skill/context, modalities), auto-suggestions, removable filter chips, a ⌘K shortcut, and faceted filters with live counts per asset type and tool. Simple phrases without facet vocabulary are searched literally; the catalog list favors title matches when ranking results. Sort by Recent, Most Used, Most Used This Week, Name, or Updated At.
...
### Smart Search
Unified bar: Gemini-powered parsing runs when the query looks like faceted search (tools, asset types, modalities); otherwise keywords stay literal. Catalog search matches titles and summaries everywhere; body text for prompts and context; skill install URL for skills. Title matches are ranked ahead of body-only matches. Auto-suggestions, relevance highlighting, removable filter chips, URL sync, debounce, and ⌘K focus.
